...
PARENTHETICAL ASIDE VALIDATION: After completing the full pass, count the parenthetical asides logged. If fewer than 3 are logged, do not output the revised script. Instead, find additional analytical or evidence sentences in the script and add more parenthetical asides until the count reaches 3. Each new aside must pass the quality standard: it reacts to what just happened, it is shorter than the sentence it interrupts, and it adds a reaction not an explanation. Only output the revised script once 3 parenthetical asides are logged and each one passes the quality check.
